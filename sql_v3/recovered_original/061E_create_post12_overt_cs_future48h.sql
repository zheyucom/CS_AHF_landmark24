...
drop table if exists study_ahf.outcome_061E_post12_overt_cs_future48h_main_v1 cascade;

create table study_ahf.outcome_061E_post12_overt_cs_future48h_main_v1 as
with base as (
    select
        b.*,
        a.deathtime,
    ...
    from study_ahf.cohort_061D_landmark12_riskset_main_v1 b
...
select
    ...
    count(*) as n,
    round(count(*) * 100.0 / sum(count(*)) over (), 2) as pct
from study_ahf.outcome_061E_post12_overt_cs_future48h_main_v1 o
...
select
    61,
    'ahf_evidence_12h',
    'study_ahf.cohort_061A_ahf_evidence_early_window_12h_v1',
    count(*),
...
from study_ahf.cohort_061A_ahf_evidence_early_window_12h_v1;
...
select
    62,
    'ahf_strict_12h',
    'study_ahf.cohort_061B_ahf_strict_12h_v1',
    count(*),
    count(distinct subject_id),
    count(distinct hadm_id),
    count(distinct stay_id),
...
select
    ...
    'study_ahf.outcome_061C_pre12_overt_cs_flags_v1',
    count(*),
    count(distinct subject_id),
    count(distinct hadm_id),
    count(distinct stay_id),
...
select
    ...
    'study_ahf.cohort_061D_landmark12_riskset_main_v1',
    count(*),
    count(distinct subject_id),
    count(distinct hadm_id),
    count(distinct stay_id),
...
from study_ahf.cohort_061D_landmark12_riskset_main_v1;
