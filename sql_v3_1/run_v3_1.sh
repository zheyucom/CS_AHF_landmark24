...
SQL_DIR="$ROOT/sql_v3_1/executable"
RUN_DIR="$ROOT/project_control/runs/20260826_v3_1_time_confirmed/sql_logs"
...
do
    ...
    "$PSQL" -X -v ON_ERROR_STOP=1 -h "$PGHOST" -U "$PGUSER" -d "$PGDATABASE" \
        ...
        echo "FAILED: $file"
        tail -40 "$RUN_DIR/$file.log"
        exit 1
    }
done

echo "[OK] v3.1 time-confirmed pipeline (001-064) completed. Logs in $RUN_DIR"
