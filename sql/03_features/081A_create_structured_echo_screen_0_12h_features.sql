...
echo_procedure_summary as (
    select
        ...
        max(case when hour_from_icu < 0 then 1 else 0 end)
            as echo_any_procedure_pre_t0_flag,
        max(case when itemid = 225432 and hour_from_icu >= 0 and hour_from_icu < 12 then 1 else 0 end)
            as tte_procedure_0_12h_flag,
        max(case when itemid = 225432 and hour_from_icu < 0 then 1 else 0 end)
            as tte_procedure_pre_t0_flag,
        max(case when itemid = 221255 and hour_from_icu >= 0 and hour_from_icu < 12 then 1 else 0 end)
            as tee_procedure_0_12h_flag,
        max(case when itemid = 221255 and hour_from_icu < 0 then 1 else 0 end)
            as tee_procedure_pre_t0_flag,
...
lvef_summary as (
    select
        ...
        count(*) filter (where charttime < intime) as structured_lvef_pre_t0_n,
        count(*) filter (where charttime >= intime) as structured_lvef_0_12h_n,
        ...
        min(charttime) filter (where charttime < intime) as first_structured_lvef_pre_t0_time,
        max(charttime) filter (where charttime < intime) as last_structured_lvef_pre_t0_time,
        min(charttime) filter (where charttime >= intime) as first_structured_lvef_0_12h_time,
        max(charttime) filter (where charttime >= intime) as last_structured_lvef_0_12h_time,
...
select
    ...
    coalesce(e.echo_any_procedure_pre_t0_flag, 0) as echo_any_procedure_pre_t0_flag,
    coalesce(e.tte_procedure_pre12_flag, 0) as tte_procedure_pre12_flag,
    coalesce(e.tte_procedure_pre_t0_flag, 0) as tte_procedure_pre_t0_flag,
    coalesce(e.tee_procedure_pre12_flag, 0) as tee_procedure_pre12_flag,
    coalesce(e.tee_procedure_pre_t0_flag, 0) as tee_procedure_pre_t0_flag,
    ...
    coalesce(l.structured_lvef_pre_t0_n, 0) as structured_lvef_pre_t0_n,
    coalesce(l.structured_lvef_0_12h_n, 0) as structured_lvef_0_12h_n,
    case when coalesce(l.structured_lvef_0_12h_n, 0) > 0 then 1 else 0 end
        as structured_lvef_0_12h_available_flag,
    ...
    l.first_structured_lvef_pre_t0_time,
    l.last_structured_lvef_pre_t0_time,
    l.first_structured_lvef_0_12h_time,
    l.last_structured_lvef_0_12h_time,
