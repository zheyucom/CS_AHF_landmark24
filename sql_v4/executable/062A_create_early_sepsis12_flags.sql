...
--
-- Input:
--   study_ahf_v4.outcome_061E_post12_overt_cs_future48h_main_v1
--
-- Main early sepsis definition:
--   sepsis3 = true, and every source record used to establish it is
--   timestamped in [ICU intime -24 h, ICU intime +12 h).
--
-- The v4 admission-present flag additionally requires suspected infection
-- before ICU admission; organ dysfunction can be confirmed during 0-12 h.
--
-- This avoids using sepsis information occurring after the 12h landmark.
-- ============================================================

drop table if exists study_ahf_v4.outcome_062A_post12_outcome_with_earlysepsis12_v1 cascade;

create table study_ahf_v4.outcome_062A_post12_outcome_with_earlysepsis12_v1 as
with base as (
    select
        *,
        intime + interval '12 hour' as landmark12_time_for_sepsis
    from study_ahf_v4.outcome_061E_post12_overt_cs_future48h_main_v1
),
...
sepsis_raw as (
    select
        ...
        case
            when s.antibiotic_time >= b.intime - interval '24 hour'
             and s.antibiotic_time <  b.intime + interval '12 hour'
            then 1 else 0
        end as antibiotic_by12_flag,

        case
            when s.culture_time >= b.intime - interval '24 hour'
             and s.culture_time <  b.intime + interval '12 hour'
            then 1 else 0
        end as culture_by12_flag,

        case
            when s.suspected_infection_time >= b.intime - interval '24 hour'
             and s.suspected_infection_time < b.intime
            then 1 else 0
        end as suspected_infection_pre_t0_flag,
...
sepsis_by_stay as (
    select
        ...
        max(
            case
                when sepsis3_flag = 1
                 ...
                 and antibiotic_by12_flag = 1
                 and culture_by12_flag = 1
                then 1 else 0
            end
        ...
        max(
            case
                when sepsis3_flag = 1
                 and suspected_infection_pre_t0_flag = 1
                 and sofa_by12_flag = 1
                 and antibiotic_by12_flag = 1
                 and culture_by12_flag = 1
                then 1 else 0
            end
        ) as early_sepsis_admission_present_flag,
        ...
        min(antibiotic_time) filter (
            where sepsis3_flag = 1
              and antibiotic_by12_flag = 1
        ) as early_sepsis12_first_antibiotic_time,

        min(culture_time) filter (
            where sepsis3_flag = 1
              and culture_by12_flag = 1
        ) as early_sepsis12_first_culture_time,
...
select
    ...
    coalesce(s.early_sepsis_admission_present_flag, 0) as early_sepsis_admission_present_flag,
    ...
    s.early_sepsis12_first_antibiotic_time,
    s.early_sepsis12_first_culture_time,
