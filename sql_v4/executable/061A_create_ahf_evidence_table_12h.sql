...
drop table if exists study_ahf_v4.cohort_061A_ahf_evidence_early_window_12h_v1 cascade;

create table study_ahf_v4.cohort_061A_ahf_evidence_early_window_12h_v1 as
with base as (
    select *
...
select
    ...
    -- These timing flags support cohort auditing only. Final ICD fields
    -- remain retrospective phenotype anchors and are excluded from modeling.
    case
        when (
            rx.iv_loop_rx_first_starttime_early12 >= b.intime - interval '24 hour'
            and rx.iv_loop_rx_first_starttime_early12 < b.intime
        ) or (
            em.loop_emar_first_charttime_early12 >= b.intime - interval '24 hour'
            and em.loop_emar_first_charttime_early12 < b.intime
        ) or (
            n.ntprobnp_first_charttime_early12 >= b.intime - interval '24 hour'
            and n.ntprobnp_first_charttime_early12 < b.intime
        ) then 1 else 0
    end as ahf_time_evidence_pre_t0_flag,

    case
        when coalesce(rx.iv_loop_rx_n_orders_early12, 0) > 0
          or coalesce(em.loop_emar_n_records_early12, 0) > 0
          or n.ntprobnp_max_early12 >= 300
        then 1 else 0
    end as ahf_time_evidence_by_t12_flag,
