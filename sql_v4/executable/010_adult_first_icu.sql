...
-- Purpose:
--   Define the adult first ICU stay per patient cohort.
--
-- Important:
...
drop table if exists study_ahf_v4.cohort_010_adult_first_icu_v1 cascade;

create table study_ahf_v4.cohort_010_adult_first_icu_v1 as
with icu_ranked as (
    select
        i.subject_id,
        i.hadm_id,
...
select
    ...
    'study_ahf_v4.cohort_010_adult_first_icu_v1' as table_name,
    count(*) as n_rows,
    count(distinct subject_id) as n_subjects,
    count(distinct hadm_id) as n_hadm,
    count(distinct stay_id) as n_stay,
    null as excluded_from_prior,
    'Adult patients, first ICU stay per patient.' as notes
from study_ahf_v4.cohort_010_adult_first_icu_v1;
