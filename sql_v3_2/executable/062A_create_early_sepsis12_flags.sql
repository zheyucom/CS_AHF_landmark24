...
--
-- Input:
--   study_ahf_v3_2.outcome_061E_post12_overt_cs_future48h_main_v1
--
...
drop table if exists study_ahf_v3_2.outcome_062A_post12_outcome_with_earlysepsis12_v1 cascade;

create table study_ahf_v3_2.outcome_062A_post12_outcome_with_earlysepsis12_v1 as
with base as (
    select
        *,
        intime + interval '12 hour' as landmark12_time_for_sepsis
    from study_ahf_v3_2.outcome_061E_post12_overt_cs_future48h_main_v1
),
...
sepsis_raw as (
    select
        ...
        -- v3.1: the antibiotic/culture pair forming the suspected
        -- infection signal must ALSO be observed before T12, otherwise
        -- the sepsis flag uses post-landmark treatment information to
        -- back-label the cohort (temporal availability leakage).
        case
            when s.antibiotic_time is not null
             and s.antibiotic_time < b.intime + interval '12 hour'
            then 1 else 0
        end as antibiotic_by12_flag,

        case
            when s.culture_time is not null
...
sepsis_by_stay as (
    select
        ...
        -- v3.1 audit: earliest antibiotic / culture pair among the
        -- sepsis3 evidence for this stay (not window-restricted here;
        -- the < T12 check is applied in the main flag).
...
from study_ahf_v3_2.outcome_062A_post12_outcome_with_earlysepsis12_v1;
