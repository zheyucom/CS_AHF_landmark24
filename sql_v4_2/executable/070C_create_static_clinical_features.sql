...
--
-- Population:
--   Strict pre-T0 AHF-only sensitivity cohort; sepsis is not an
--   inclusion criterion. Sepsis timing/severity fields, when present,
--   are retained as optional 0-12h predictors.
--   landmark 12h
...
drop table if exists study_ahf_v4_2.model_070C_static_clinical_features_v1 cascade;

create table study_ahf_v4_2.model_070C_static_clinical_features_v1 as
with base as (
    select *
...
select
    ...
    extract(epoch from (
        b.early_sepsis12_first_suspected_infection_time - b.intime
    )) / 3600.0 as early_sepsis12_suspected_infection_hour,

    extract(epoch from (
        b.early_sepsis12_first_sofa_time - b.intime
    )) / 3600.0 as early_sepsis12_sofa_hour,

    case
        when b.early_sepsis12_first_suspected_infection_time < b.intime then 1
        else 0
    end as suspected_infection_before_icu_flag,

    case
        when b.early_sepsis12_first_suspected_infection_time >= b.intime
         and b.early_sepsis12_first_suspected_infection_time < b.intime + interval '6 hour'
        then 1 else 0
    end as suspected_infection_icu_0_6h_flag,

    case
        when b.early_sepsis12_first_suspected_infection_time >= b.intime + interval '6 hour'
         and b.early_sepsis12_first_suspected_infection_time < b.intime + interval '12 hour'
        then 1 else 0
...
from study_ahf_v4_2.model_070C_static_clinical_features_v1;
