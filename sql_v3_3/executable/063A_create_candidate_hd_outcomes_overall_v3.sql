-- Revised candidate hemodynamic-deterioration path, v3.
-- Lactate rollups use exact analysis_value; censored results only support
-- one-sided "definitely above" threshold statements.
-- Sample time and result-availability time must fall in the same pre/post-T12
-- window so a late-reported pre-T12 sample cannot leak into the baseline.

drop table if exists study_ahf_v3_3.outcome_063A_candidate_hd_outcomes_overall_v3 cascade;

create table study_ahf_v3_3.outcome_063A_candidate_hd_outcomes_overall_v3 as
with base as (
    select
        e.*,
...
from study_ahf_v3_3.outcome_063A_candidate_hd_outcomes_overall_v3;
