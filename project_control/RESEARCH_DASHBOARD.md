...
| MIMIC实验室正式流水线 | V2 实际聚合与不可变快照审计已通过；阶段 A 静态质量门已登记 167/167 SQL，并把全部旧执行链 fail-closed | 阶段 B 建立 raw 合同层并复制修订主线/敏感性 SQL；数据库 QC 通过后逐文件晋级 ACTIVE | 8-16 h；数据库复核另计 | 高 |
